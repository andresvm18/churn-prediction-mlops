...
def encode_input_data(input_data):
    ...
    if categorical_encoders is None:
        raise RuntimeError(
            "Label encoders not found. Prediction service unavailable."
        )
...
def get_recommendation(risk_level):
    ...
    if risk_level == "Medium":
        return (
            ...
            "Monitor behavior and engagement."
        )
...
def predict_customer_churn(customer_data):
    ...
    if churn_model is None:
        raise RuntimeError(
            "Model files not found. Prediction service unavailable."
        )
    ...
    churn_probability = (
        churn_model.predict_proba(encoded_data)[0][1]  # Step 4: Get probability
    )
    ...
    prediction_label = (
        "Churn"
        if prediction == 1
        else "No Churn"                                 # Step 6: Human-readable label
    )
