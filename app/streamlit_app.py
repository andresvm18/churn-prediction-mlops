...
with left_col:
    ...
    with tab3:
        ...
        with c2:
            total_charges = st.number_input("Total charges ($)", min_value=0.0, value=1100.0, step=10.0)
...
with right_col:
    ...
    if st.session_state.has_prediction and st.session_state.last_prediction and not st.session_state.is_loading:
        ...
        if st.session_state.top_factors:
            ...
            badges = "".join(f'<span class="ci-badge">{f}</span>' for f in st.session_state.top_factors)
...
# ─── Footer ───────────────────────────────────────────────────────────────────
st.html('<div class="ci-footer">Churn Intelligence · Powered by Machine Learning · v2.1</div>')